        ...
        private void BtnOnayla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        // Log kaydet
                        string logMessage = $"{txtunvan.Text} firması {Session.KullaniciAdi} tarafından onaylandı";
                        if (!string.IsNullOrWhiteSpace(txtbşdetay.Text))
                        {
                            logMessage += $" - Not: {txtbşdetay.Text}";
                        }
                        DatabaseHelper.LogIslem(logMessage);
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (affected > 0)
                    {
                        ...
                        DatabaseHelper.LogIslem(
                            $"{txtunvan.Text} firması {Session.KullaniciAdi} tarafından reddedildi - Neden: {txtbşdetay.Text}");
        ...
        private void BtnGoruntule_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show($"'{listBelge.SelectedItem}' belgesi görüntüleniyor...\n" +
                    "(Bu özellik belge yükleme sistemi kurulduğunda aktif olacaktır)", 
                    "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
